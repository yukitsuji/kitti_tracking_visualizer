...
#define HEIGHT 1.71 // the height of camera
#define THETA -0.015     // the ground plane pitch angle
...
int main(int argc, char *argv[])
{
    ...
    const std::vector<mono_3d_det::Tracklet> tracklets = mono_3d_det::ParseLabelFile(labelFilePath);
    ...
    Eigen::MatrixXd tmp_affineYAxisMatrix(4, 4);
    while (1)
    {
        /*
        ...
        */
        ...
        for (unsigned int obj_i = 1; obj_i < tracklets.size(); obj_i++)
        {
            ...
            if (tracklets[obj_i].obj_type == "DontCare")
                continue;
            if (tracklets[obj_i].obj_type == "Truck")
                continue;
            ...
            tmp_affineYAxisMatrix = affineYAxisMatrix;
            // ================================================
